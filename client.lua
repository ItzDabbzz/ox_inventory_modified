...
local currentWeapon
local Categories = lib.load('data.categories')
...
function client.openInventory(inv, data)
	...
	left.cash = pd.money and pd.money.cash or 0
	left.bank = pd.money and pd.money.bank or 0

	left.charinfo = {
		firstname = pd.charinfo and pd.charinfo.firstname or 'You',
		lastname = pd.charinfo and pd.charinfo.lastname or 'Failed',
		birthdate = pd.charinfo and pd.charinfo.birthdate or '00/00/0000',
		gender = pd.charinfo and pd.charinfo.gender or 0,
		nationality = pd.charinfo and pd.charinfo.nationality or 'American',
		cid = pd.citizenid or 'FAILED',
		char_id = pd.charinfo and pd.charinfo.cid or 'FAILED',
		backstory = pd.charinfo and pd.charinfo.backstory or 'N/A',
		health = pd.metadata and pd.metadata.health or 100,
		armor = pd.metadata and pd.metadata.armor or 0,
		...
		bloodtype = pd.metadata and pd.metadata.bloodtype or 'FAILED',
		fingerprint = pd.metadata and pd.metadata.fingerprint or 'FAILED'
	}
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	-- Prepare leftInventory data
	local leftInventoryData = {
		id = cache.playerId,
		slots = shared.playerslots,
	...
	-- Always add charinfo for initial setup since there's no right inventory context here
	leftInventoryData.charinfo = {
		firstname = pd.charinfo and pd.charinfo.firstname or '',
		lastname = pd.charinfo and pd.charinfo.lastname or '',
		birthdate = pd.charinfo and pd.charinfo.birthdate or '',
		gender = pd.charinfo and pd.charinfo.gender or 0,
		nationality = pd.charinfo and pd.charinfo.nationality or '',
		cid = pd.citizenid or '',
		char_id = pd.charinfo and pd.charinfo.cid or '',
		backstory = pd.charinfo and pd.charinfo.backstory or '',
		health = pd.metadata and pd.metadata.health or 100,
		armor = pd.metadata and pd.metadata.armor or 0,
		hunger = pd.metadata and pd.metadata.hunger or 100,
		thirst = pd.metadata and pd.metadata.thirst or 100,
		stress = pd.metadata and pd.metadata.stress or 0,
		bloodtype = pd.metadata and pd.metadata.bloodtype or 'O+',
		fingerprint = pd.metadata and pd.metadata.fingerprint or ''
	}
	...
	SendNUIMessage({
		...
		data = {
			...
			leftInventory = leftInventoryData,
