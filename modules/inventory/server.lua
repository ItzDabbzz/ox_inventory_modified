...
function Inventory.SetSlot(inv, item, count, metadata, slot)
	...
	if currentSlot and newCount < 1 then
		...
		currentSlot = {name = item.name, label = item.label, weight = item.weight, slot = slot, count = newCount, description = item.description, metadata = metadata, stack = item.stack, close = item.close, category = item.category}
...
local function generateItems(inv, invType, items)
	...
	for i = 1, #items do
		...
		if not item then
			...
			returnData[i] = {name = item.name, label = item.label, weight = weight, slot = i, count = count, description = item.description, metadata = metadata, stack = item.stack, close = item.close, category = item.category}
		end
	end
	...
	if result then
		...
		for _, v in pairs(result) do
			...
			if item then
				...
				weight += slotWeight
				returnData[v.slot] = {name = item.name, label = item.label, weight = slotWeight, slot = v.slot, count = v.count, description = item.description, metadata = v.metadata, stack = item.stack, close = item.close, category = item.category}
			end
		end
	end
